...
function generateIcons(src, projectPath) {
    ...
    const tasks = Object.keys(icons).map((key) => {
        ...
        return resizeImage(src, path.join(targetPath, `${key}-icon.png`), size, size);
    });
...
function generateScreens(src, projectPath) {
    ...
    Object.keys(screens).forEach((key) => {
        ...
        tasks.push(resizeImage(src, filePath, width, height));
        tasks.push(resizeImage(src, filePathPort, height, width));
    });
    return Promise.all(tasks);
}
